 ...
 * 
 * Project Url: http://work.xuld.net/tealsharp
 * 
 * This source code is part of the Project Teal.
 * 
...
namespace Teal.Utility.Json {
    ...
    public sealed class JsonUndefined : IJsonConverter {
        ...
        /// 从指定 JSON 读取器读取对象。
        /// </summary>
        /// <param name="engine">正在使用的 JSON 引擎。</param>
        /// <param name="reader">要读取的读取器。</param>
        /// <param name="type">要读取的类型。</param>
        /// <returns>返回读取的对象。</returns>
        object IJsonConverter.read(JsonEngine engine, JsonReader reader, Type type) {
        ...
        void IJsonConverter.write(JsonEngine engine, JsonWriter writer, object value) {
    ...
    public sealed class JsonWriter : IDisposable {
        ...
        public void writeDateTime(DateTime value) {
            output.Write('\"');
            output.Write(value.Year);
            output.Write('/');
            writePaddedInt(output, value.Month);
            output.Write('/');
            writePaddedInt(output, value.Day);
            output.Write(' ');
            writePaddedInt(output, value.Hour);
            output.Write(':');
            writePaddedInt(output, value.Minute);
            output.Write(':');
            writePaddedInt(output, value.Second);
            output.Write('\"');
        }

        static void writePaddedInt(TextWriter output, int value) {
            if (value < 10) {
                output.Write('0');
                output.Write((char)(value + '0'));
                return;
            }
            output.Write(value);
        }
    ...
    public static class Json {
        ...
        /// <summary>
        /// 获取或设置当前用于解析 JSON 的引擎。
        /// </summary>
        public static JsonEngine currentEngine = new JsonEngine();

        /// <summary>
        /// 获取所有转换器。
        /// </summary>
        public static Dictionary<Type, IJsonConverter> converters {
            get {
                return currentEngine.converters;
            }
        }
        ...
        public static object parse(string jsonString) {
            return jsonString == null ? null : currentEngine.parse(jsonString);
        }
        ...
        public static object parse(string jsonString, bool strictMode) {
            return jsonString == null ? null : currentEngine.parse(jsonString, strictMode);
        }
        ...
        public static object parse(JsonReader reader) {
            return currentEngine.parse(reader);
        }
        ...
        public static object parse(string jsonString, Type targetType) {
            return jsonString == null ? null : currentEngine.parse(jsonString, targetType);
        }
        ...
        public static object parse(string jsonString, Type targetType, bool strictMode) {
            return jsonString == null ? null : currentEngine.parse(jsonString, targetType, strictMode);
        }
        ...
        public static string stringify(object value) {
            return currentEngine.stringify(value);
        }
        ...
        public static string stringify(object value, bool indent) {
            return currentEngine.stringify(value, indent);
        }
    ...
    public sealed class JsonEngine {
        ...
        /// 获取所有转换器。
        /// </summary>
        public readonly Dictionary<Type, IJsonConverter> converters = new Dictionary<Type, IJsonConverter>() {
            ...
            {typeof(IDataReader), new DataReaderJsonConverter()},
        ...
        public IJsonConverter getConverter(Type type) {
            ...
            if (typeof(IEnumerable).IsAssignableFrom(type)) {
                ...
                changedType = typeof(IDataReader);
            } else {
        ...
        public object parse(string jsonString) {
            return parse(new JsonReader(new StringReader(jsonString)));
        }
        ...
        /// <returns>返回一个 JSON 对象。</returns>
        /// <exception cref="JsonParseException">解析错误时发生的异常。</exception>
        public object parse(string jsonString, bool strictMode) {
            return parse(new JsonReader(new StringReader(jsonString)) {
        ...
        /// <returns>返回一个 JSON 对象。</returns>
        /// <exception cref="JsonParseException">解析错误时发生的异常。</exception>
        public object parse(string jsonString, Type targetType) {
            return parse(new JsonReader(new StringReader(jsonString)), targetType);
        }
        ...
        /// <returns>返回一个 JSON 对象。</returns>
        /// <exception cref="JsonParseException">解析错误时发生的异常。</exception>
        public object parse(string jsonString, Type targetType, bool strictMode) {
            return parse(new JsonReader(new StringReader(jsonString)) {
                strictMode = strictMode
            }, targetType);
        }
        ...
        /// <returns>返回一个 JSON 对象。</returns>
        /// <exception cref="JsonParseException">解析错误时发生的异常。</exception>
        public object parse(JsonReader reader, Type targetType) {
            object result;
            if (reader.readValueStart()) {
        ...
        /// <returns>返回读取的对象。</returns>
        /// <exception cref="JsonParseException">解析错误时发生的异常。</exception>
        public object read(JsonReader reader, Type targetType) {
            return getConverter(targetType).read(this, reader, targetType);
        }
        ...
        /// <param name="indent">指示是否格式化输出的 JSON 字符串。</param>
        /// <returns>JSON 字符串。</returns>
        public string stringify(object value, bool indent) {
            var sb = new StringBuilder();
        ...
        public void write(JsonWriter writer, object value) {
            ...
            try {
                ...
                converter.write(this, writer, value);
            } finally {
    ...
    public interface IJsonConverter {
        ...
        /// <param name="type">要读取的类型。</param>
        /// <returns>返回读取的对象。</returns>
        object read(JsonEngine engine, JsonReader reader, Type type);
        ...
        /// <param name="writer">要写入的输出器。</param>
        /// <param name="value">要写入的值。</param>
        void write(JsonEngine engine, JsonWriter writer, object value);

    }

    sealed class DefaultJsonConverter : IJsonConverter {

        public object read(JsonEngine engine, JsonReader reader, Type type) {

            // 如果期待类型为 JSON 本身，则直接读取。
            if (type == typeof(object)) {
                return engine.read(reader);
            }
            ...
            if (reader.currentTokenType == JsonTokenType.objectStart) {
                ...
                while (reader.readObjectItemStart(out propertyName)) {
                    ...
                    prop.SetValue(r, read(engine, reader, prop.PropertyType), null);
                    if (reader.readObjectItemEnd()) {
                        break;
                    }
                }
        ...
        public void write(JsonEngine engine, JsonWriter writer, object value) {
            ...
            for (int i = 0; i < fields.Length; i++) {
                ...
                engine.write(writer, p.GetValue(value));
            }
            ...
            for (int i = 0; i < properties.Length; i++) {
                ...
                var p = properties[i];
                writer.writePropertyName(p.Name);
                engine.write(writer, p.GetValue(value, null));
            }
            writer.indent--;
            if (appendComma) {
                writer.writeLine();
    ...
    abstract class EnumerableJsonConverterBase {
        ...
        protected void readItems(JsonEngine engine, JsonReader reader, Type elementType, IList arr) {
            bool foundUndefined;
            while (reader.readArrayItemStart(out foundUndefined)) {
                arr.Add(foundUndefined ? Activator.CreateInstance(elementType) : engine.read(reader, elementType));
                if (reader.readArrayItemEnd()) {
                    break;
                }
            }
    ...
    sealed class EnumerableJsonConverter : EnumerableJsonConverterBase, IJsonConverter {
        ...
        public object read(JsonEngine engine, JsonReader reader, Type type) {
            if (reader.currentTokenType == JsonTokenType.arrayStart) {
                ...
                readItems(engine, reader, elementType, arr);
                return arr;
            }
            engine.read(reader);
            return null;
    ...
    sealed class DictionaryBasedJsonConverter : IJsonConverter {
        ...
        public object read(JsonEngine engine, JsonReader reader, Type type) {
            if (reader.currentTokenType == JsonTokenType.objectStart) {
                ...
                string propertyName;
                while (reader.readObjectItemStart(out propertyName)) {
                    obj[propertyName] = engine.read(reader, elementType);
                    if (reader.readObjectItemEnd()) {
                        break;
                    }
                }
    ...
    sealed class DataSetJsonConverter : EnumerableJsonConverterBase, IJsonConverter {
        ...
        public object read(JsonEngine engine, JsonReader reader, Type type) {
            if (reader.currentTokenType == JsonTokenType.arrayStart) {
                ...
                bool foundUndefined;
                while (reader.readArrayItemStart(out foundUndefined)) {
                    ds.Tables.Add(foundUndefined ? new DataTable() : (DataTable)engine.read(reader, typeof(DataTable)));
                    if (reader.readArrayItemEnd()) {
                        break;
                    }
                }
        ...
        public new void write(JsonEngine engine, JsonWriter writer, object value) {
            base.write(engine, writer, ((DataSet)value).Tables);
        }
        ...
        public void write(JsonEngine engine, JsonWriter writer, object value) {
            ...
            for (int i = 0; i < dt.Rows.Count; i++) {
                ...
                for (int j = 0; j < dt.Columns.Count; j++) {
                    ...
                    engine.write(writer, dt.Rows[i][j]);
                }
                writer.indent--;
    ...
    sealed class DataReaderJsonConverter : IJsonConverter {

        public object read(JsonEngine engine, JsonReader reader, Type type) {
            throw new InvalidOperationException("无法读取 JSON 为 " + type.FullName + " 类型");
        }
