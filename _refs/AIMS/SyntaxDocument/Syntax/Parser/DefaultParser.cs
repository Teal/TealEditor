...
namespace AIMS.Libraries.CodeEditor.Syntax.SyntaxDocumentParsers {
    ...
    public sealed class DefaultParser : IParser {
        ...
        //om denna 鋜 true
        // s?ska INTE n鋝ta rad parse'as , utan denna ska fixas s?den blir som den f鰎ra... (kopiera segment)
        ...
        private void InternalParseLine(int index, bool ParseKeywords) {
            ...
            //k鰎 tills vi kommit till slutet av raden..
            ...
            while (true) {
                ...
                // 在当前区块内部，找到当前区块的终止符或新区块的开始符。
                // 比如 } 和 #endregion 都是当前区块的终止符。
                ...
                // 如果发现了 }, 则 EndSegment 指向它。
                // 如果发现了 // ，则 ChildSegment 指向它。
        ...
        /* private void ParseSegments(Row Row)
         {
             //skapa segment f鰎 raden och s鋞t start och endsegment f鰎 raden
             Row.Clear();
             strinBg k = Row.Text;
