...
namespace AIMS.Libraries.Scripting.Dom.ReflectionLayer
{
	public static class ReflectionReturnType
	{
		...
		{
			return ReflectionReturnType.Create(@class.ProjectContent, @class, type, createLazyReturnType);
		}

		public static IReturnType Create(IMember member, Type type, bool createLazyReturnType)
		{
			return ReflectionReturnType.Create(member.DeclaringType.ProjectContent, member, type, createLazyReturnType);
		}
		...
		public static IReturnType Create(IProjectContent pc, IDecoration member, Type type, bool createLazyReturnType)
		{
			...
			if (type.IsByRef)
			{
				result = ReflectionReturnType.Create(pc, member, type.GetElementType(), createLazyReturnType);
			}
			else if (type.IsArray)
			{
				result = new ArrayReturnType(pc, ReflectionReturnType.Create(pc, member, type.GetElementType(), createLazyReturnType), type.GetArrayRank());
			}
			else if (type.IsGenericType && !type.IsGenericTypeDefinition)
			{
				...
				{
					para.Add(ReflectionReturnType.Create(pc, member, args[i], createLazyReturnType));
				}
				result = new ConstructedReturnType(ReflectionReturnType.Create(pc, member, type.GetGenericTypeDefinition(), createLazyReturnType), para);
			}
			else if (type.IsGenericParameter)
			{
				...
				if (c != null && type.GenericParameterPosition < c.TypeParameters.Count)
				{
					...
					{
						result = new GenericReturnType(c.TypeParameters[type.GenericParameterPosition]);
						return result;
					}
				}
				if (type.DeclaringMethod != null)
				{
					...
					if (method != null)
					{
						...
						{
							result = new GenericReturnType(method.TypeParameters[type.GenericParameterPosition]);
							return result;
						}
						result = new GenericReturnType(new DefaultTypeParameter(method, type));
						return result;
					}
				}
				result = new GenericReturnType(new DefaultTypeParameter(c, type));
			}
			else
			{
				...
				result = new GetClassReturnType(pc, name, typeParameterCount);
			}
			return result;
		}
	}
